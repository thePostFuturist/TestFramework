...
## TDD with Prefabs Pattern

### Overview

When testing Unity prefabs in a TDD environment, follow these principles:
1. **Editor Scripts Create Prefabs** - Use Editor scripts to programmatically create prefabs with all references properly assigned
2. **Tests Instantiate in Setup** - Tests instantiate the prefab during setup phase
3. **Cleanup in Teardown** - Always destroy test instances in teardown
4. **Separate Directories** - Keep Editor scripts and test scripts in different directories for clean organization
...
│   │       └── MyPrefabFactory.cs
│   └── Tests/
│       └── PlayMode/               # Test scripts
│           └── MyPrefabTests.cs
└── Resources/
    └── TestPrefabs/                # Generated prefabs stored here
```

### Editor Script for Prefab Creation

Create an Editor script that programmatically builds your prefab with all references:
...
1. **Always Use Editor Scripts for Prefab Creation**
   - Ensures consistency across test runs
   - Makes prefab configuration version-controlled
   - Allows easy recreation if prefab is lost
